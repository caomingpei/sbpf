...
macro_rules! declare_builtin_function {
    // NovaFuzz, To match cpi_common, we need to pass in the instrumenter
    ...
    // NovaFuzz, To match SyscallTryFindProgramAddress
    ($(#[$attr:meta])* $name:ident $(<$($generic_ident:tt : $generic_type:tt),+>)?, fn rust(
        $vm:ident : &mut $ContextObject:ty,
        $arg_a:ident : u64,
        $arg_b:ident : u64,
        ...
        $arg_e:ident : u64,
        $memory_mapping:ident : &mut $MemoryMapping:ty,
        $instrument_collector:ident : &mut DataCollector,
    ) -> $Result:ty { $($rust:tt)* }) => {
        $(#[$attr])*
        pub struct $name {}
        impl $name {
